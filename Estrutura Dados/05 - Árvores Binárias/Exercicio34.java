			...
			if(node != null){
				if(element == node.getElement()) return	true;
					...
					if(element < node.getElement()) {
						if(this.isElementAtTree(element, node.getLeft())) return true;
					}
					else{
						if(isElementAtTree(element, node.getRight())) return true;
					}
				}
			}
			return	false;
	...
	public static void main(String[] args) {
		...
		while (option != 'd') {
			...
			case 'c':
				...
				boolean found = tree.isElementAtTree(num1 , tree.raiz);
				
				if(found) System.out.println("O elemento: " + num1 + " foi encontrado na árvore");
				if(!found) System.out.println("O elemento: " + num1 + " não foi encontrado na árvore");
